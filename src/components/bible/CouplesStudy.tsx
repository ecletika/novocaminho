              ...
              <div key={topic.id} className="space-y-2">
                <div className="flex items-center group">
                  <button onClick={() => toggleTopic(topic.id)} className={`flex-1 flex items-center justify-between p-4 rounded-2xl transition-all text-left ${expandedTopics[topic.id] ? 'bg-[#1E40AF] text-white shadow-lg' : 'hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-400'}`}>
                    <span className="text-sm font-bold break-words flex-1 flex items-center gap-3">
                      <span className={`text-[10px] w-6 h-6 rounded-full flex items-center justify-center font-black shrink-0 ${expandedTopics[topic.id] ? 'bg-white/20 text-white' : 'bg-gray-100 dark:bg-gray-800'}`}>
                        {topic.position}
                      </span>
                      {topic.title}
                    </span>
                    ...
                    {(lessons[topic.id] || []).map(lesson => {
                      ...
                      return (
                        <div key={lesson.id} className="flex items-center group/lesson">
                          <button onClick={() => { setSelectedLesson(lesson); setShowPdfReader(false); }} className={`flex-1 p-3.5 rounded-xl text-xs text-left transition-all overflow-hidden ${selectedLesson?.id === lesson.id ? 'bg-[#1E40AF]/10 text-[#1E40AF] font-black' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50'}`}>
                            <div className="flex items-start gap-3">
                              <BookOpen size={14} className={`mt-0.5 shrink-0 ${selectedLesson?.id === lesson.id ? 'text-[#1E40AF]' : 'opacity-30'}`} />
                              <span className={`break-words flex-1 ${isCompleted ? 'line-through opacity-50' : ''}`}>{lesson.title}</span>
          ...
          <article className="bg-white dark:bg-gray-900 rounded-[3rem] p-6 md:p-16 shadow-2xl border border-gray-100 dark:border-gray-800 min-h-full animate-in fade-in slide-in-from-bottom-4 text-left relative overflow-hidden flex flex-col">
            ...
            <div className="flex-1 relative z-10">
              ...
              <div className="prose prose-stone dark:prose-invert lg:prose-2xl max-w-none mb-16">
                <div className="font-serif text-xl md:text-2xl leading-[1.8] text-gray-700 dark:text-gray-300 whitespace-pre-wrap italic first-letter:text-5xl first-letter:font-bold first-letter:text-[#1E40AF] first-letter:mr-2">
